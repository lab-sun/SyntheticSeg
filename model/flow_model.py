...
from model.mysegformer.mybuilder import EncoderDecoder as segmodel
...
class FlowModel(nn.Module):
    def __init__(self, student, criterion, args):
        ...
        checkpoint = torch.load(
            "./pretrained/FlowNet2_checkpoint.pth.tar")
    ...
    def inference(self, image):
        ...
        scale_pred = F.upsample(input=preds, size=(w, h), mode='bilinear', align_corners=True)

        return scale_pred

    def cal_loss(self, rgb_frame, s_frames, rgb_image, s_image, label):
        self.student.train()
        self.flownet.eval()
        ...
        scale_pred = F.upsample(input=preds, size=(w, h), mode='bilinear', align_corners=True)
        scale_frames = F.upsample(input=preds_frame, size=(w, h), mode='bilinear', align_corners=True)
        ...
        with torch.no_grad():
            flow_i21 = self.flownet(rgb_image, rgb_frame)
        ...
        ST_loss = 0.1 * self.criterion_flow(scale_pred * noc_mask2, warp_o1 * noc_mask2)
    ...
    def forward(self, rgb_frame, s_frames, rgb_image, s_image, label, mode):
        if mode == 'train':
            ret = self.cal_loss(rgb_frame, s_frames, rgb_image, s_image, label)
        elif mode == 'test':
...
if __name__ == '__main__':
    ...
    student = segmodel(cfg=config, criterion=criterion, encoder_name='mit_b2', decoder_name='MLPDecoder', norm_layer=nn.BatchNorm2d).cuda()
